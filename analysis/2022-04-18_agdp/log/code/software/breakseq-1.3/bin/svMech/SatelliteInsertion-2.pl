...
open SV_REPEAT,">$out_dir/Output/Insertions/$ARGV[1].VNTRInsertions-2.txt" or die;  #label
open Other, ">$out_dir/Output/Insertions/$ARGV[1].NonVNTR_Insertions-2.txt" or die;

print SV_REPEAT "chr\tStart\tend\tInserted_seq\tVNTRname\tVNTRclass\tVNTRid\tVNTRlen\tSV_Size\tRatio\tID\n";
print Other "chr\tStart\tend\tInserted_seq\tID\n";
<SV>;
while(<SV>){
	$num_SV++;
	chomp;
	@fields = ();
	@fields = split/\s+/, $_;
	(open REPEATMASKER,"<$out_dir/RepeatMasker_Output/Insertions/Qchr$fields[0]_$fields[1]-$fields[2].$fields[4].txt.out")or die "Cannot open repeatmasker Insertions/Qchr$fields[0]_$fields[1]-$fields[2].$fields[4].txt.out!\n";
	$templine= <REPEATMASKER>;
	@temp = ();
	@temp = split/\s+/, $templine;
 	if(($temp[0] eq "There")&&($temp[1] eq "were")&&($temp[2] eq "no")){
		print Other "$fields[0]\t$fields[1]\t$fields[2]\t$fields[3]\t$fields[4]\n";
		close REPEATMASKER;
 # system "rm ./RepeatMasker_Output/Insertions/Qchr$fields[0]_$fields[1]-$fields[2].txt*";
		next;
	}
	<REPEATMASKER>;
	<REPEATMASKER>;
	...
	if($mark==0){
		...
		print SV_REPEAT "$fields[0]\t$fields[1]\t$fields[2]\t$fields[3]\t$VNTRname\t$VNTRclass\t$VNTRID\t$VNTR_len\t$SV_Size\t",($VNTR_len/$SV_Size),"\t$fields[4]\n";
	}	
}
