...
DATADIR=/home/jon/Desktop/Breakseq_allINVs/Data/datos_librerias
WORKDIR=/home/jon/Desktop/Breakseq_allINVs/Results/
...
# ref.header.template => lo cogemos de Crick, no hace falta crearlo
...
for i in `cat $WORKDIR/individuals_set1.txt`; do
        ...
        if [ -d $i ] && [ ! -e $i/ref.header ]; then
                ...
                echo
                #gawk -v ID=$i '{gsub(/XXX/, ID); print}' $DATADIR/ref.header.template > $i/ref.header
	...
	if [ -d $i ] && [ ! -e $i/header ]; then
                ...
                echo "Creating more headers for $i."
                echo
                #gawk -v ID=$i '{gsub(/XXX/, ID); print}' $DATADIR/bplib.header.template > $i/header
