...
#This program uses the -100~+100 bp sequences of the left & right breakpoint of the Deletions in the Novel Genome
...
open NAHR, ">$out_dir/Output/Deletions/$ARGV[1].NAHR_Deletions.txt";
open OTHER, ">$out_dir/$ARGV[1].Other_Non_NAHR_Deletions.txt";
open NOTSURE,">$out_dir/Output/Deletions/$ARGV[1].UNSURE_3_OffsetOrNOtSpanBP_In_NAHR_Deletions.txt";
open NOTSURE2,">$out_dir/Output/Deletions/$ARGV[1].UNSURE_3_OffsetOrNOtSpanBP_In_NAHR_Deletions_bl2seqDetail.txt";
open NOTSURE3,">$out_dir/Output/Deletions/$ARGV[1].UNSURE_4_Potential_NAHR_Deletions.txt";
open NOTSURE4,">$out_dir/Output/Deletions/$ARGV[1].UNSURE_4_Potential_NAHR_Deletions_bl2seqDetail.txt";
...
while($line1=<FLANKINGSEQ>){
	...
	if($numbers[0] eq ""){
		...
		if($SV_Size < 2*$length_f){
			open IN, "<$out_dir/RepeatMasker_Output/Deletions/Qchr$fields[$Chr]_$fields[$Start]-$fields[$End].$fields[$ID].txt" or die;
			<IN>;
			$seq = <IN>;
			chomp($seq);
			close IN;
