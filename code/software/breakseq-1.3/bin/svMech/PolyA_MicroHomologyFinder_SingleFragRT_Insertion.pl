...
open FLANKINGSEQ, "<$ARGV[0]"; #Modified06072009.Single_Retrotransposon_Insertions.txt
open MICROHOMOLOGY, ">$out_dir/Output/Insertions/$ARGV[1].MicroHomologySingleFragRT_Insertions.txt";
open NOTSURE, ">$out_dir/Output/Insertions/$ARGV[1].UNSURE_5_Single_Retrotransposon_Insertions.txt";

print MICROHOMOLOGY "ChrNo.\tStart\tEnd\tLeftFlankingSequence\tRightFlankingSequence\tName\tInsertedSeq\tID\tcoverage\tfull\tratio\ttherFull\tratio2\tfinal\tClass\tNames\tnmhNo.\tnmhSeq\tpolyAonCorrStrand\tStrandElement\tTSD\n";
print NOTSURE "ChrNo.\tStart\tEnd\tLeftFlankingSequence\tRightFlankingSequence\tName\tInsertedSeq\tID\tcoverage\tfull\tratio\ttherFull\tratio2\tfinal\tClass\tNames\tnmhNo.\tnmhSeq\tpolyAonCorrStrand\tStrandElement\tTSD\n";
<FLANKINGSEQ>;
while(<FLANKINGSEQ>){
	chomp;
	@fields = ();
	...
	if(!($element eq "")){
		if(($markTsd == 1)&($pA ==1)){
			print MICROHOMOLOGY "$fields[$Chr]\t$fields[$Start]\t$fields[$End]\t$fields[$Left_F]\t$fields[$Right_F]\t$fields[$Name]\t$fields[$InsertedSeq]\t$fields[$ID]\t$coverage\t$full\t",($coverage/$full),"\t$ther_full\t",($coverage/$ther_full),"\t$final\t$element\t$fields[$position3]\t$mhNum\t$mhSeq\t$pA\t$fields[$position]\t$markTsd\n";
		}else{
			print NOTSURE "$fields[$Chr]\t$fields[$Start]\t$fields[$End]\t$fields[$Left_F]\t$fields[$Right_F]\t$fields[$Name]\t$fields[$InsertedSeq]\t$fields[$ID]\t$coverage\t$full\t",($coverage/$full),"\t$ther_full\t",($coverage/$ther_full),"\t$final\t$element\t$fields[$position3]\t$mhNum\t$mhSeq\t$pA\t$fields[$position]\t$markTsd\n";
		}  
	}else{
...
open FLANKINGSEQ, "<$ARGV[2]"; #Modified06072009.WholeMultiple_Retrotransposon_Insertions_Format3.txt
<FLANKINGSEQ>;
